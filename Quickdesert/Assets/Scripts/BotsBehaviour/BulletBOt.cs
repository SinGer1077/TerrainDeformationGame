...
public class BulletBOt : MonoBehaviour
{
    ...
    private void Explode()
    {
        if (alreadyExpload == false)
        {
            ...
            {               
                enemies[i].gameObject.GetComponent<PlayerGameplay>().TakeDamage(explosionDamage);
            }
            // Collider[] enemies = Physics.OverlapSphere(transform.position, explosionRange, whatIsEnemies);
            Invoke("Delay", 0.05f);
        }
    ...
    {
        collisions++;
        if ((collision.collider.CompareTag("Terrain") || collision.collider.CompareTag("Player")) && explodeOnTouch)
            Explode();
    }
